...
from chszlablib import Graph, Decomposition, PartitionResult
...
class TestKaffpaE:
    def test_basic(self):
        g = make_bipartite()
        result = Decomposition.evolutionary_partition(g, num_parts=2, time_limit=1, mode="fast")
    ...
    def test_modes(self):
        ...
        for mode in ["fast", "eco", "strong"]:
            result = Decomposition.evolutionary_partition(g, num_parts=2, time_limit=1, mode=mode)
            assert result.edgecut >= 0
            assert len(result.assignment) == 6
    ...
    def test_k_parts(self):
        g = make_bipartite()
        result = Decomposition.evolutionary_partition(g, num_parts=3, time_limit=1, mode="fast")
        assert len(result.assignment) == 6
    ...
    def test_warm_start(self):
        ...
        r1 = Decomposition.evolutionary_partition(g, num_parts=2, time_limit=1, mode="fast", seed=42)
        # Warm-start from the first result
        r2 = Decomposition.evolutionary_partition(
