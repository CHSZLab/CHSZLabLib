// Fake mpi.h — single-process stub for ScalableCorrelationClustering.
// Placed on the include path BEFORE system headers so #include <mpi.h> finds
// this instead of a real MPI installation.
#ifndef PSEUDO_MPI_H
#define PSEUDO_MPI_H
...
// Point-to-point communication — no-ops for single process
...
inline int MPI_Iprobe(int source, int tag, MPI_Comm comm, int *flag, MPI_Status *status) {
    *flag = 0;
    if (status) { status->MPI_SOURCE = 0; status->MPI_TAG = 0; status->MPI_ERROR = MPI_SUCCESS; status->_count = 0; status->_cancelled = 0; }
    return MPI_SUCCESS;
}
...
inline int MPI_Wait(MPI_Request *request, MPI_Status *status) {
    if (status) { status->MPI_SOURCE = 0; status->MPI_TAG = 0; }
    return MPI_SUCCESS;
}
